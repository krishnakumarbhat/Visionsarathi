...
def process_image_and_send_text(num, client_socket):
    ...
    # Write the output properly
    output = f"this is image captioning output with ocr in it  can you write it poperly (  {image_caption} and  text :{ocr_text}) for the blind person discrib it in short(15 words) with text written in it"
    # print(output)

    # Invoke Ollama for text processing
    llm = Ollama(model="llama2")
    processed_text = llm.invoke(output)
    print(processed_text)

    # Send combined text back to client
    client_socket.sendall(processed_text.encode())
